...
class MaskedCouplingLayer(nn.Module):
    ...
    def forward(self, z):
        ...
        """
        scaled_z = self.scale_net(self.mask * z)
        z_ = self.mask * z + (1 - self.mask) * (z * torch.exp(scaled_z) + self.translation_net(self.mask * z))
        log_det_J = torch.sum((1-self.mask)*scaled_z, dim=1)
        return z_, log_det_J
    ...
    def inverse(self, z_):
        ...
        """
        scaled_z_ = self.scale_net(self.mask * z_)
        z = self.mask * z_ + (1 - self.mask) * ((z_ - self.translation_net(self.mask * z_)) * torch.exp(-scaled_z_))
        log_det_J = -torch.sum((1-self.mask)*scaled_z_, dim=1)
        return z, log_det_J
...
class FlattenedMNIST(datasets.MNIST):
    def __getitem__(self, index):
        # The parent MNIST returns (image, label)
        img, _ = super().__getitem__(index)
        return img


def get_checkerboard_mask():
    # Create a 28x28 grid of indices
    mask = torch.zeros((28, 28))
    for i in range(28):
        for j in range(28):
            # If the sum of coordinates is even, set to 1
            if (i + j) % 2 == 0:
                mask[i, j] = 1
    return mask.flatten() # Returns shape [784]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='flow_model.pt', help='file to save model to or load model from (default: %(default)s)')
    ...
    parser.add_argument('--ntransformations', type=int, default=5, metavar='N', help='number of transformations in the flow (default: %(default)s)')
    parser.add_argument('--nhidden', type=int, default=8, metavar='N', help='number of hidden units in each transformation (default: %(default)s)')
    ...
    match args.data:
        ...
        case 'mnist':
            ...
            train_loader = torch.utils.data.DataLoader(FlattenedMNIST('data/', train=True, download=True, transform=transform), batch_size=args.batch_size, shuffle=True)
            test_loader = torch.utils.data.DataLoader(FlattenedMNIST('data/', train=False, download=True, transform=transform), batch_size=args.batch_size, shuffle=True)
    ...
    num_transformations = args.ntransformations
    ...
    if args.mode == 'train':
        ...
        # Save model
        torch.save(model.state_dict(), f"exercises/models/{args.model}")

    elif args.mode == 'sample':
        import matplotlib.pyplot as plt
        import numpy as np

        model.load_state_dict(torch.load(f"exercises/models/{args.model}", map_location=torch.device(args.device)))
        ...
        if args.data == 'mnist':
            ...
            save_image(samples, f"exercises/samples/{args.model}-{args.samples}", nrow=8)
            print(f"Samples saved to exercises/samples/{args.model}-{args.samples}")
        else:
            ...
            ax.set_aspect('equal')
            # fig.colorbar(im)
            plt.savefig(f"exercises/samples/{args.model}-{args.samples}")
            plt.close()